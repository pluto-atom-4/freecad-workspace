...
def main() -> int:
    ...
    log_event("Phase 13", f"URDF path: {URDF_FILE}")
    log_event("Phase 13", f"PROTO path: {PROTO_FILE}")

    print("=== Phase 13: PROTO Structure Validation ===\n")

    # Check input files exist
    if not URDF_FILE.exists():
        log_event("Phase 13", "ERROR: URDF file not found")
        print(f"ERROR: URDF file not found at {URDF_FILE}")
        return 1

    if not PROTO_FILE.exists():
        log_event("Phase 13", "ERROR: PROTO file not found")
    ...
    if urdf_root is None:
        log_event("Phase 13", "ERROR: Failed to parse URDF XML")
        return 1

    proto_content = load_proto(PROTO_FILE)
    if proto_content is None:
        log_event("Phase 13", "ERROR: Failed to load PROTO file")
        return 1

    log_event("Phase 13", f"✓ Loaded URDF from {URDF_FILE}")
    log_event("Phase 13", f"✓ Loaded PROTO from {PROTO_FILE}")
    ...
    log_event("Phase 13", "Running validation checks...")
    ...
    for check in checks:
        ...
        log_event("Phase 13", f"Check '{check_name}': {status} ({details})")
    ...
    if summary["failed_checks"] > 0:
        log_event("Phase 13", f"VALIDATION FAILED: {summary['failed_checks']}/{summary['total_checks']} check(s) failed")
        print(f"ERROR: {summary['failed_checks']} validation check(s) failed")
        return 1

    log_event("Phase 13", f"✓ PHASE 13 SUCCESS: All {summary['total_checks']} checks passed")
    log_event("Phase 13", "========== PHASE 13: PROTO STRUCTURE VALIDATION SUCCESS ==========")

...
**Collision-detection logic:** the controller watches its own wheel-position telemetry for a plateau — once at least 10 samples have been recorded, if the left wheel's position changed by less than `0.01` rad over the last 5 samples, it records that moment as `collision_detected_time`/`collision_detected_rotation`. Telemetry prints roughly every 2 simulated seconds as `T=<t>s [MOVING|COLLISION] | L_pos=<rad> | R_pos=<rad>`.

**Verdict logic (plain `Robot`, wheel-position telemetry only):** PASS requires the robot to have moved forward initially (checked via the first 10 samples), a collision plateau to have been detected before `max_time = 10.0` s, and the two wheels to still be in sync (`|left_final - right_final| <= 0.1` rad) at the end — it does **not** independently confirm chassis position/contact via a `Supervisor` check, so a false plateau (e.g. from an unrelated physics stall) would be indistinguishable from a genuine collision.
